  ...
  for(ss in 1:4){
    for(m in 1:2){
      for(s in 1:2){
        ...
        while(vareta > 1){
          # latent variables
          theta = mvtnorm::rmvnorm(sample_size, rep(0,factor_N), vcov.theta)
          
          #indicators 
          X = matrix(NA, sample_size, item_total)
          #Y = matrix(NA, sample_size, 16)
          ...
          for(f in 1:factor_N){
            for(r in 1:item_N){
              j = j + 1
              X[ , j] = mx[j] + lambda[j] * theta[, f] + rnorm(sample_size, mean = 0, sd = sqrt(1 - lambda[j]^2))
            }
          }
        ...
        Y <- eta + rnorm(sample_size, mean = 0, sd = sqrt(1 - vareta))
        
        #lambday = runif(16, min = 0.2, max = 0.7)
        
        #for(r in 1:16){
        #  Y[ , r] = my[r] + lambday[r] * eta + rnorm(sample_size, mean = 0, sd = sqrt(1 - lambday[r]^2))
        #}
